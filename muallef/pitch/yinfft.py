from muallef.util.units import dB_to_power
...
def yinfft_detect(signal, sampleRate, weight, tolerance=0.85):
    ...
    #im = np.append(im, 0)
    spectrum = rfft(windowed_signal, len(signal))

    # square magnitude of the spectrum, with applied weights, and calculate its sum
    #weight = spectral_weights(length, sampleRate)
    #sqr_mag = (spectrum**2) * weight
    #sqr_mag = np.append(sqr_mag[:-1], sqr_mag[:0:-1])
    first = (spectrum.real[0]**2) * weight[0]
    tmp = spectrum.real[1:]**2 + spectrum.imag[1:]**2
    ...
    fftout = rfft(sqr_mag)
    fft = np.append(fftout.real, fftout.imag[::-1])
