...
class PitchKlapuri(object):
    ...
    def _freq_to_note(self, freq):
        i_note = np.argmin(np.abs(PitchKlapuri.frequencies-freq))
        return PitchKlapuri.notes[i_note]
...
def estimate_multipitch(signal, sampleRate, polyphony_max=6):
    freq_est = PitchKlapuri(max_poly=polyphony_max)
